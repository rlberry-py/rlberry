    ...
    if (self.doneLoading == 0):
      ...
      if self.self_collision:
        ...
        self.objects = self._p.loadMJCF(
            os.path.join(data.getDataPath(), "mjcf", self.model_xml, flags = pybullet.URDF_GOOGLEY_UNDEFINED_COLORS))
        self.parts, self.jdict, self.ordered_joints, self.robot_body = self.addToScene(
            self._p, self.objects)
...
class URDFBasedRobot2(URDFBasedRobot):
  def __init__(self,
    ...
    super().__init__(model_urdf, robot_name, action_dim, obs_dim, basePosition, baseOrientation, fixed_base, self_collision)
    self.doneLoading = 0
