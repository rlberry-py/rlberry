...
def test_experiment_manager_and_multiple_managers_No_seeding(env, agent_class):
    experiment_manager = ExperimentManager(
        agent_class, env, fit_budget=2, init_kwargs={}, n_fit=6
    )
    experiment_manager_test = ExperimentManager(
        agent_class, env, fit_budget=2, init_kwargs={}, n_fit=6
    )
    ...
    for ii in range(2, experiment_manager.n_fit):
        ...
        assert not compare_trajectories(traj1, traj3)
        assert not compare_trajectories(traj2, traj3)
        assert not compare_trajectories(traj1, traj1_test)
        assert not compare_trajectories(traj2, traj2_test)
        assert not compare_trajectories(traj3, traj3_test)
